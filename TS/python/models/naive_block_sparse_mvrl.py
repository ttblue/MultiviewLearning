...
class NBSMVRLConfig(object):
  def __init__(
      ...
      lambda_global=1.0, verbose=True):
...
class NaiveBlockSparseMVRL(object):
  ...
  def _block_sparse_obj_for_view(self, vi):
    ...
    rest_data = np.concatenate(rest_data_list)
    ...
    error_obj = cvx.norm(rest_data * p_var - vi_data)
  ...
  def _optimize(self):
    ...
    for vi in range(self._nviews):
      ...
      projs = np.split(vi_vars[vi].value, split_inds, axis=1)
      self.view_projections[vi] = {
          vo: pr for vo, pr in zip(other_views, projs)
      }
